...
let toxicdevil = Config.WORKTYPE == 'public' ? false : true
   ...
   Julie.addCommand({pattern: 'spdf ?(.*)', fromMe: toxicdevil, desc: 'Converts Site to PDF.' }, (async (message, match) => {
    ...
    await message.client.sendMessage(message.jid, '```Converting Site to PDF please wait..```', MessageType.text);

    await message.sendMessage(Buffer.from(webimage.data), MessageType.document, {mimetype: Mimetype.pdf, filename: 'ahmad.pdf'});
